...
// Enhanced WebXR AR Modal Component with proper camera display
const ARModal = ({ isOpen, onClose, size, materialColor, multiplier }) => {
  const [arSupported, setArSupported] = useState(false);
  const [arSession, setArSession] = useState(null);
  const [arStatus, setArStatus] = useState('Checking AR support...');
  ...
  const startARSession = async () => {
    try {
      ...
      setArSession(session);
      setArStatus('AR Active - Point camera at flat surface');
      setIsPlacing(true);

      // Initialize proper AR rendering with visible canvas
      await initializeARRendering(session);
      ...
      session.addEventListener('end', () => {
        ...
        // Clean up canvas
        if (arCanvas && arCanvas.parentNode) {
          arCanvas.parentNode.removeChild(arCanvas);
        }
        setArCanvas(null);
      });
  ...
  const initializeARRendering = async (session) => {
    try {
      // Create and setup AR canvas
      const canvas = document.createElement('canvas');
      ...
      const onXRFrame = (time, frame) => {
        ...
        // Clear and render AR frame
        gl.bindFramebuffer(gl.FRAMEBUFFER, baseLayer.framebuffer);
        gl.clearColor(0, 0, 0, 0);
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

        // Update status based on hit test
        if (hitTestResults.length > 0) {
          if (arStatus === 'AR Active - Point camera at flat surface') {
            setArStatus('Surface detected! Tap to place veranda');
          }
          renderARIndicator(gl, hitTestResults[0], referenceSpace, pose);
        } else {
          if (arStatus === 'Surface detected! Tap to place veranda') {
            setArStatus('AR Active - Point camera at flat surface');
          }
        }

        // Render placed objects
        renderPlacedObjects(gl, pose, referenceSpace);
      };
      ...
      canvas.addEventListener('touchstart', (event) => {
        ...
        if (hitTestResults.length > 0 && isPlacing) {
          ...
          if (hitPose) {
            ...
            setArStatus(`${placedBoxes.length + 1} veranda${placedBoxes.length === 0 ? '' : 's'} placed`);
          }
        }
      }, { passive: false });
      ...
      setArStatus('Failed to initialize AR rendering');
    }
  };

  const renderARIndicator = (gl, hit, referenceSpace, viewerPose) => {
    // Simple indicator rendering - in a real app you'd use a proper 3D engine
    // This shows where the veranda would be placed
    
    // Basic circle indicator at hit position
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    
    // You would implement actual 3D rendering here
    // For now, we at least have the WebXR session running with camera
  };

  const renderPlacedObjects = (gl, viewerPose, referenceSpace) => {
    // Render all placed veranda objects
    placedBoxes.forEach(box => {
      // Basic box rendering at placed positions
      // In a real implementation, you'd render the actual 3D veranda model
    });
  };
